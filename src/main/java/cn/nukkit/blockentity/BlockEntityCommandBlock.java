...
public class BlockEntityCommandBlock extends BlockEntitySpawnable implements ICommandBlock, BlockEntityNameable {
                ...
                if (!Strings.isNullOrEmpty(cmd)) {
                    ...
                    if (this.getSuccessCount() > 0) {
                        ...
                        if (block instanceof BlockCommandChain) {
                            (((BlockCommandChain) block).getBlockEntity()).trigger(++chain);
                        }
                    }
                }
    ...
    public int getMode() {
        ...
        if (block.getId() == REPEATING_COMMAND_BLOCK) {
            return MODE_REPEATING;
        } else if (block.getId() == CHAIN_COMMAND_BLOCK) {
            return MODE_CHAIN;
        }
        return MODE_NORMAL;
    }
    ...
    public boolean setConditionMet() {
        Block block;
        if (this.isConditional() && (block = this.getBlock()) instanceof BlockCommand) {
            Block next = block.getSide(((Faceable) block).getBlockFace().getOpposite());
            if (next instanceof BlockCommand) {
                BlockEntityCommandBlock commandBlock = ((BlockCommand) next).getBlockEntity();
